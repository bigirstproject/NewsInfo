...
import com.sunsun.framework.component.base.AppException;
import com.sunsun.framework.component.base.BaseApplication;
import com.sunsun.framework.component.debug.KGLog;
import com.sunsun.framework.component.utils.UmengUtil;
...
public class KGHttpClient {
	 ...
	 */
	public static void request(RequestPackage requestPackage,
			ResponsePackage<Object> responsePackage, boolean isRetry)
			throws AppException {
			...
			try {
				...
				if (statusCode != HttpStatus.SC_OK
					...
					if (responsePackage != null) {
						responsePackage.setResponseError(httpMethod
								.getResponseBody());
					}
	...
	private static HttpMethod createHttpMethod(RequestPackage requestPackage) {
		...
		if (requestPackage.getRequestType() == RequestPackage.TYPE_GET) {
			...
			((PostMethod) httpMethod).setRequestEntity(requestPackage
		...
		if (settings != null) {
			if (settings.containsKey("socket-timeout")) {
				// 设置 请求超时时间
				httpMethod.getParams().setSoTimeout(
