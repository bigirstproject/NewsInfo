 ...
 * </p>
 * 使用： LCache static lCache = new LCache(); ... String imgUrl =
 * "http://monstar.ch/uploads/img/201212/22085340_Zun9.jpg";
 * if(lCache.isCached(imgUrl)){ Bitmap tBitmap = lCache.get(imgUrl); }else{
 * //从服务端获取图像 Bitmap tBitmap = 服务端获取的Bitmap lCache.put(imgUrl,tBitmap); }
 * 
 * @author lei.guoting
 */
public class LCache {
    private static final int DEFAULT_MAX_SIZE = (int) (1024 * 1024 * 3.5f); // 默认缓存池大小
                                                                            // 3.5M
    ...
    /**
     * @param key key值
     * @return true-key对应的Bitmap在该缓存池中 false-key对应的Bitmap不在该缓存池中
     */
